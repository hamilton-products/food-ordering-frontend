...
function SidebarWithSearch({ orderDetails, restaurantDetails }) {
  ...
  return (
    <Card className="h-[calc(100vh-3rem)] w-full max-w-full sm:max-w-[30rem] sm:min-w-[30rem] md:max-w-[40rem] md:min-w-[40rem] lg:max-w-[40rem] lg:min-w-[40rem] p-4 shadow-xl shadow-blue-gray-900/5 overflow-y-auto rounded-none"
    style={{
      background: "#F4F5F5",
      scrollbarWidth: "none", 
      ...
      <div className="container mx-auto grid grid-cols-1 gap-x-10 gap-y-5 md:grid-cols-1 xl:grid-cols-1">
        <div className="w-full py-5 px-5 mb-5 mt-5">
          <Stepper activeStep={activeStep} className="grid gap-y-10 lg:gap-y-5 grid-cols-2 lg:grid-cols-4 place-items-center" >
            <Step>
              {activeStep >= 0 ? (
