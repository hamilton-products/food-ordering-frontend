...
//               <Marker.AdvancedMarkerElement position={center} />
//             </GoogleMap>
//           </div>
//         </div>
//       </div>
...
function Product({ restaurantDetails }) {
  ...
  const { isLoaded, loadError } = useJsApiLoader({
    googleMapsApiKey: "AIzaSyDV3aChbZOKFp2kMd2Z-KCE_oeAzDVvlco", // Replace with your API key
    libraries: ["places", "marker"], // Load required libraries
    version: "weekly",
  });
  ...
  useEffect(() => {
    if (isLoaded) {
      ...
      markerElement.innerHTML = `
        <div style="padding: 10px; background: #fff; border: 1px solid #ccc; border-radius: 10px;">
  ...
  return (
    <Card
      className="h-[calc(100vh)] w-full p-4 shadow-xl overflow-y-auto"
      style={{ background: "#F4F5F5", scrollbarWidth: "none" }}
    >
      <div className="absolute z-10 mt-1">
        <Button color="black" variant="text" onClick={handleBackButton}>
          <ArrowLeftIcon className="h-8 w-8 " />
        </Button>
      </div>
      ...
      <LocationSelector/>
      <div className="mb-3">
        {isLoaded && <div id="map" style={{ height: "400px", width: "100%" }} />}
      </div>
